...
这个配置的逻辑是：Tumblr 端用 `xXx.tumblr.com` 这种形式，`blog.xXx.com` 则指向 Nginx 所在的服务器。当浏览器访问 `blog.xXx.com` 时，Nginx 用代理的方式把 `xXx.tumblr.com` 的内容呈现出来并替换所有链接。(这个方案有一个「不完美」的地方，后面再说。)  
